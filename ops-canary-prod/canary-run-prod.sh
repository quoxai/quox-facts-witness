#!/bin/bash
# ops-canary-prod daily driver: checkpoint -> TSA countersign -> push.
# Invoked by the quox-ops-canary-prod systemd user timer (Persistent=true, so a
...
node canary-checkpoint-prod.mjs
...
if [ -f "$DAY_DIR/statement.tsr" ]; then
  echo "canary-prod: $DAY_DIR already TSA-countersigned (no-op)"
else
